...
2. https://github.com/UCFoxi/CallStack-Spoofer
...
DWORD GetTargetThreadIDFromProcName(const wchar_t* ProcName)
{
	...
	while (retval)
	{
		...
		{
			char buf[MAX_PATH] = { 0 };
			GetFullPathNameA("C:\\Users\\Desktop\\internal_p2c.dll", MAX_PATH, buf, NULL);
...
BOOL AttemptMulti(LPSTR MutexName)
{
	...
	if (hMutex != NULL)
	{
		DWORD pID = FindProcessId(L"RustClient.exe");
		printf("pID: %d\n", pID);
...
int main(int argc, char** argv)
{
	...
	bool injected = false;
	std::cout << "[INFORMATION] MAKE SURE TO HAVE GAMEWER OPEN IN THE BACKGROUND BEFORE YOU OPEN THE LOADER" << std::endl;
	std::cout << "[INFORMATION] - > Waiting 20 seconds in total" << std::endl;
	Sleep(10000);
	EnableDebugPriv();
	...
	{
		// Retrieve process ID 
		DWORD pID = GetTargetThreadIDFromProcName(L"RustClient.exe");
		

		if (pID)
		{
			std::cout << "8 Second Timer" << std::endl;
			Sleep(8000);
			injected = true;
			printf("Injected!\n");
		}
